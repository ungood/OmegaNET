...
{
    [TestFixture]
    public class ExtendedCharsFormatterFixture
    {
        private static readonly ExtendedCharsFormatter extended = new ExtendedCharsFormatter();

        private void AssertExtended(string extendedChar, byte expected)
        {
            var expStr = string.Format("\x08{0}", (char) expected);
            var actual = extended.Format(extendedChar);
            Assert.AreEqual(expStr, actual);
        }

        [Test]
        public void TestAllExtendedChars()
        {
